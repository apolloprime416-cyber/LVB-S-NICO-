...
function spTemplateLicenseGate() {
  return '<div class="sp-license-gate">' +
    '<img class="sp-login-logo" src="/icons/icon128.png" alt="' + ((window.tsBrandName && window.tsBrandName()) || 'Painel Lovable') + '">' +
    '<p class="sp-gate-title">Bem vindo a <span data-ts-brand="name">' + ((window.tsBrandName && window.tsBrandName()) || 'Painel Lovable') + '</span></p>' +
    '<p class="sp-gate-desc">Insira sua chave de licença para desbloquear.</p>' +
    '<input class="sp-input" id="sp-license-input" placeholder="SORAX-XXXXXXXXXXXXXXX" spellcheck="false">' +
    ...
    '<div class="sp-gate-actions">' +
      ...
      (function(){
        ...
        var defaultMsg = 'Olá! Preciso de suporte para a extensão ' + ((window.tsBrandName && window.tsBrandName()) || 'Painel Lovable') + '.';
        var href = url;
        try {
...
'</button>' +
    ...
    '<div id="sp-reseller-btn" style="display:none;margin-bottom:14px">' +
      '<a href="https://extensaosorax.lovable.app/painelrevenda" target="_blank" style="display:flex;align-items:center;gap:8px;padding:10px 14px;border-radius:10px;border:1px solid rgba(124,90,255,0.3);background:rgba(124,90,255,0.06);color:var(--ql-accent);text-decoration:none;font-size:12px;font-weight:700;transition:all 0.2s">' +
