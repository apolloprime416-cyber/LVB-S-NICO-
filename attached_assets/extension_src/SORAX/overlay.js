...
(function () {
  ...
  function injectGlobalStyles() {
    ...
    style.textContent = `
      ...
      #${SUBMENU_ID} .ts-fab-item:hover .ts-fab-circle {
        box-shadow: 0 6px 16px rgba(124, 90, 255, 0.55) !important;
      }
  ...
  async function fetchNotifications() {
    return await bgFetch(NOTIFICATIONS_URL, {
      method: "GET",
  ...
  async function openNotificationsPanel() {
    ...
    panel.innerHTML = '<div class="ts-notif-head"><span>Notificações</span><button class="ts-notif-close" type="button">✕</button></div><div class="ts-notif-list"><p class="ts-notif-empty">Carregando...</p></div>';
  ...
  // with the Painel Lovable sender identity. We only touch leaf text nodes so
  // we never break Lovable's interactive controls.
  function getTsHeaderLabel() {
    return "⚡ Enviado por SORAX";
  }
