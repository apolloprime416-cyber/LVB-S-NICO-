...
const VALIDATE_URL = "https://hyjsaialebpskwfvinig.supabase.co/functions/v1/validate-license";
const OPTIMIZE_URL = "https://hyjsaialebpskwfvinig.supabase.co/functions/v1/optimize-prompt";
const NOTIFICATIONS_URL = "https://hyjsaialebpskwfvinig.supabase.co/rest/v1/notifications?select=*&order=created_at.desc&limit=20";
...
var VERSIONS_URL_POPUP = "https://hyjsaialebpskwfvinig.supabase.co/rest/v1/extension_versions?select=version,changelog,file_path,is_alert_active&order=created_at.desc&limit=1&is_alert_active=eq.true";
...
async function checkForUpdatePopup() {
  try {
    ...
    if (latest.version !== CURRENT_EXT_VERSION_POPUP && latest.is_alert_active) {
      ...
      if (banner) {
        var dlUrl = latest.file_path ? "https://hyjsaialebpskwfvinig.supabase.co/storage/v1/object/public/extension-releases/" + latest.file_path : null;
    ...
    function playRemoteSound(type, volume){
      try {
        ...
        if (p && p.catch) p.catch(function(err){
          console.warn("[Painel Lovable] Falha ao reproduzir som:", err);
        });
      } catch(err){
        console.warn("[Painel Lovable] Falha ao reproduzir som:", err);
      }
